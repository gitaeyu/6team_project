...
class Kwangju_Restaurant(QWidget,form_kwangju):
    ...
    def searching_data(self): # 검색 눌렀을때 테이블 위젯에 데이터 입력
        ...
        self.data_list=[] # word1,word2,word3에 해당하는 data값 append하기 위해 빈 리스트 생성
        for i in self.kwangju_list:
            if word1 in i[7]:
                if word2 in i[3]:
                    if word3 in i[4]:
                        if word4 in i[1]:
                            ...
                            pass
        ## 테이블위젯에 데이터 입력
        self.store_table.setRowCount(len(self.data_list))
        self.store_table.setColumnCount(12)
        for j in range(len(self.data_list)):
            for k in range(0, 12):
                self.store_table.setItem(j, k, QTableWidgetItem(str(self.data_list[j][k])))
